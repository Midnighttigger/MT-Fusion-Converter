...
for i in range(len(f)):
    ...
    try:
        ...
        if exporttype == 1:
            ...
            tx39.close()
            tx40 = tf.crop((0,base*4,base,base*5))
            tx40.save("export/" + index + "/35.png")
            tx40.close()
            tx41 = tf.crop((base*2,base*4,base*3,base*5))
            tx41.save("export/" + index + "/11.png")
            tx41.close()
            tx42 = tf.crop((base*3,base*4,base*4,base*5))
            tx42.save("export/" + index + "/10.png")
            tx42.close()
            tx43 = tf.crop((base*4,base*4,base*5,base*5))
            tx43.save("export/" + index + "/8.png")
            tx43.close()
            tx44 = tf.crop((base*5,base*4,base*6,base*5))
            tx44.save("export/" + index + "/20.png")
            tx44.close()
            tx45 = tf.crop((base*6,base*4,base*7,base*5))
            tx45.save("export/" + index + "/44.png")
            tx45.close()
            tx46 = tf.crop((base*7,base*4,base*8,base*5))
